...
from engine.map import redraw_map, get_character_rect, map_data
...
def ground_roam(direction = 's', num = 0):
    ...
    for row in range(len(map_data)):
        for col in range(len(map_data[0])):
            if map_data[row][col][0] == '#':
                barrier_rect = pygame.Rect(col * cell_size, row * cell_size, cell_size, cell_size)
    ...
    if not collision or encounter:
        redraw_map(new_character_rect, direction, num)

        return (encounter, direction, num)

    redraw_map(get_character_rect(), direction, 0)
